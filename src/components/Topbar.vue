    ...
    <header
        ...
        <nav class="hidden md:flex items-center gap-1 flex-1">
            <RouterLink v-for="v in VIEWS.filter(v => v.id !== 'about')" :key="v.id" :to="{ name: v.id }"
                class="font-cinzel text-sm tracking-[0.08em] uppercase px-3 py-1.5 transition-all border border-transparent"
        ...
        <div class="flex items-center gap-2">
            ...
            <a :href="ABOUT.cvUrl" download
                class="hidden md:flex items-center gap-1.5 font-cinzel text-sm font-bold tracking-wider text-[#080610] bg-gold px-4 py-2 ml-1 hover:bg-gold2 transition-all [clip-path:polygon(5px_0%,100%_0%,calc(100%-5px)_100%,0%_100%)]">
                <Download :size="16" />
                Télécharger le CV
            </a>
